  ...
  return (
    <View style={{marginTop:30,backgroundColor:'pink',height:90,bottom:30}}>
      <View style={{flexDirection:'row',justifyContent:'space-around',marginTop:40,alignItems:'center'}}>
        ...
        <Text style={{fontSize:20,color:'black'}}>페이지 작성</Text>
...
export default function PagePost( {navigation,route } ) { 
    ...
    const Save = async () => {
      ...
      if (image != null) {
        // 이미지 업로드를 위한 경로 설정
        const childPath = `${firebase.auth().currentUser.uid + Math.random().toString(36) + "pp"}`;
       ...
       // Firestore에 글 데이터 저장
      firebase.firestore().collection("PagePosts")
      .doc()
      // .collection("userPosts")
      .set({
        NotepostId:Math.random().toString(36) + "fdp",
        NoteId:route.params.Pageid,
      ...
      .then(function () {
        ...
        // 이전 화면으로 돌아가기
        navigation.goBack(route.params.Pageid)
      });
    }
    // 취소 팝업
    const cancelPopup = () => {
    ...
    return (
      <View style={{ flex: 1, backgroundColor: 'white' }}>
        ...
        {/* 로딩 중인 경우 로딩 표시 */}
        {loading && <ActivityIndicator style={{ height: 40, marginLeft: 10, bottom: 0 }} size="large" color="#000000" />}
