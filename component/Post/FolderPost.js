  ...
  return (
    <View style={{marginTop:30,backgroundColor:'pink',height:90,bottom:30}}>
      <View style={{flexDirection:'row',justifyContent:'space-around',marginTop:40,alignItems:'center'}}>
        ...
        <Text style={{fontSize:20,color:'black'}}>글 작성</Text>
...
export default function FolderPost( {navigation,props,route } ) { 
    ...
    const Save = async () => {
      ...
      firebase.firestore().collection("FolderPosts")
        .doc()
        .set({
          folderpostId: Math.random().toString(36) + "fdp",
